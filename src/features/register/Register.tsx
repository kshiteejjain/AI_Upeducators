...
const Register = () => {
    ...
    const sendEmail = async (email: string, otp: number) => {
        try {
            await emailjs.send(import.meta.env.VITE_EMAILJS_SERVICE_ID, import.meta.env.VITE_EMAILJS_TEMPLATE_SEND_OTP, {
                ...formData,
                message: otp,
                to_email: email,
    ...
    const formSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (await isEmailInPaidUsers(formData.email, 'Silver')) {
                plan = 'Silver';
                formData.total_credits += 1000;
                formData.remain_credits += 1000;
