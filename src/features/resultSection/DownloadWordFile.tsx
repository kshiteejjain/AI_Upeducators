  ...
  const handleExportDoc = () => {
    ...
    const formatText = (text: string) => {
      ...
      for (let i = 0; i < parts.length; i++) {
        if (numberRegex.test(parts[i]) && i > 0) {
          // Only insert a line break if the previous part didn't end with a full stop after a number
        ...
        if (numberRegex.test(parts[i].trim())) {
          // Append the part that is a number followed by a dot and space directly
